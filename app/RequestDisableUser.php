...
class RequestDisableUser extends Model
{
    use SoftDeletes;

    protected $table = 'request_disable_user';
    ...
    {
        $data = User
            ::leftJoin('request_disable_user', 'users.id', '=', 'request_disable_user.user_id')
            ->leftJoin('positions', 'users.position_id', '=', 'positions.id')
            ->leftJoin('approve', 'users.id', '=', 'approve.reviewer_id')
            ->where('approve.request_id', $this->id)
            ->where('approve.type', config('app.type_request_disable_user'))
    ...
    {
        $total = DB::table('request_disable_user')
                ->where('request_disable_user.status', config('app.approve_status_approve'))
                ->whereNull('request_disable_user.deleted_at')
                ->count();
        return $total;
    }
    ...
    {
        $total = DB::table('request_disable_user')
                ->where('request_disable_user.status', config('app.approve_status_draft'))
                ->whereNull('request_disable_user.deleted_at')
                ->count();
        return $total;
    }
    ...
    {
        $total = DB::table('request_disable_user')
                ->where('request_disable_user.status', config('app.approve_status_reject'))
                ->whereNull('request_disable_user.deleted_at')
                ->count();
        return $total;
    }
    ...
    {
        $total = DB::table('request_disable_user')
                ->whereNotNull('request_disable_user.deleted_at')
                ->count();
        return $total;
    }
    ...
    {
        $pending = config('app.approve_status_draft');
        $typeMemo = config('app.type_request_disable_user');
        $totalApproval = DB
            ::table('request_disable_user')
            ->join('approve', 'request_disable_user.id', '=', 'approve.request_id')
            ->where('approve.type', '=', $typeMemo)
            ->where('approve.reviewer_id', '=', Auth::id());
        if ($status) {
            $totalApproval = $totalApproval->where('request_disable_user.status', '=', $status);
        }
        if ($approveStatus) {
    ...
    public static function totalReject($status = 3, $approveStatus = 3)
    {
        ...
        $totalApproval = DB
            ...
            ->join('approve', 'request_disable_user.id', '=', 'approve.request_id')
            ->where('approve.type', '=', $typeMemo)
            ->where('request_disable_user.user_id', '=', Auth::id());
        if ($status) {
            $totalApproval = $totalApproval->where('request_disable_user.status', '=', $status);
        }
        if ($approveStatus) {
...
//        dd($total);
    ...
    public static function pendingList()
    {
        ...
        $data = DB::table('request_disable_user')
            ->join('users', 'users.id', '=', 'request_disable_user.user_id')
            ->leftJoin('approve', 'request_disable_user.id', '=', 'approve.request_id');

            if (Auth::user()->role !== 1) {
                $data = $data->where('request_disable_user.user_id', '=', Auth::id());

            }
            $data = $data->where('request_disable_user.status', '=', $pending);
        $data = $data
            ...
            ->distinct('request_disable_user.id')
            ->get();

        $type = config('app.type_request_disable_user');
        $data1 = DB::table('request_disable_user')
            ->leftJoin('approve', 'request_disable_user.id', '=', 'approve.request_id')
            ->leftJoin('users', 'users.id', '=', 'request_disable_user.user_id')
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('request_disable_user.status', '=', $pending)
            ...
            ->groupBy('request_disable_user.id')
            ->get();
    ...
    public static function presidentpendingList($company)
    {
        ...
        $data = DB::table('request_disable_user')
            ...
            ->leftJoin('approve', 'request_disable_user.id', '=', 'approve.request_id')
            ->where('approve.type', '=', $type)
            ->where('request_disable_user.company_id', '=', $company);
            if (Auth::user()->role !== 1) {
                $data = $data->where('request_disable_user.user_id', '=', Auth::id());
    ...
    public static function presidentApproved($company, $department = null)
    {
        ...
        $data = $data->where('request_disable_user.status', '=', $approved);
        $data = $data
            ->whereNull('deleted_at')
            ->select(
                'request_disable_user.*',
        ...
        $data1 = DB::table('request_disable_user')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('request_disable_user.status', '=', $approved);
    ...
    public static function rejectedList()
    {
        ...
        $data = DB::table('request_disable_user')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('request_disable_user.status', '=', $reject)
            ->whereNull('deleted_at')
            ->select(
                'request_disable_user.*',
                'users.name as requester_name',
    ...
    public static function presidentRejectedList($company)
    {
        ...
        $data1 = DB::table('request_disable_user')
            ...
            if (Auth::user()->role !== 1) {
                $data1 = $data1->where('request_disable_user.user_id', '=', Auth::id());
            }
    ...
    public static function CountPending($company)
    {
        ...
        $data = $data
            ->where('request_disable_user.status', '=', $status)
            ->whereNull('deleted_at')
            ->select(
                'request_disable_user.id'
            )
            ->distinct('request_disable_user.id')
            ->get();
        ...
        if (Auth::user()->position->level == config('app.position_level_president')) {
            foreach ($data as $key => $item) {
                $approveData = Approve::where('type', $type)
                    ...
                    ->select('id')
                    ->get();
                $relatedRequest = Approve::where('type', config('app.type_request_disable_user'))
                    ->where('request_id', $item->hr_id)
                    ->where('reviewer_id', '!=', getCEO()->id)
                    ->whereNotIn('approve.position', ['cc'])
                    ->whereIn('status', [$pending, $reject])

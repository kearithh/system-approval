...
class ReturnBudget extends Model
{
    use SoftDeletes;

    protected $table = 'return_budget';
    ...
    {
        $data = User
            ::leftJoin('return_budget', 'users.id', '=', 'return_budget.user_id')
            ->leftJoin('positions', 'users.position_id', '=', 'positions.id')
            ->leftJoin('approve', 'users.id', '=', 'approve.reviewer_id')
            ->where('approve.request_id', $this->id)
            ->where('approve.type', config('app.type_return_budget'))
    ...
    public function approver()
    {
        $data = User
            ...
            ->select([
                ...
                'return_budget.id as request_id',
                'return_budget.user_id as request_user_id',
                'return_budget.status as request_status',
                'approve.user_object',
    ...
    public function approvals()
    {
        $approvals = DB
            ::table('return_budget')
            ->join('approve', 'return_budget.id', '=', 'approve.request_id')
    ...
    public static function presidentpendingList($company)
    {
        ...
        $data = DB::table('return_budget')
            ->join('users', 'users.id', '=', 'return_budget.user_id')
            ->leftJoin('approve', 'return_budget.id', '=', 'approve.request_id')
            ->where('return_budget.company_id', '=', $company)
            ->where('approve.type', '=', $type);
        if (Auth::user()->role !== 1) {
            $data = $data->where('return_budget.user_id', '=', Auth::id());
        }

        $data = $data->where('return_budget.status', '=', $pending);
        $data = $data
            ...
            ->distinct('return_budget.id')
            ->get();

        $data1 = DB::table('return_budget')
            ->leftJoin('approve', 'return_budget.id', '=', 'approve.request_id')
            ->leftJoin('users', 'users.id', '=', 'return_budget.user_id')
            ->where('approve.type', '=', $type)
            ->where('return_budget.company_id', '=', $company)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('return_budget.status', '=', $pending)
    ...
    public static function presidentApprove($company)
    {
        ...
        $data = DB::table('return_budget')
            ...
            ->whereNotIn('return_budget.status', [config('app.approve_status_reject'), config('app.approve_status_disable')])
            ->whereNull('deleted_at')
            ->select(
                'return_budget.*',
                'users.name as requester_name',
    ...
    public static function presidentApproved($company, $department = null)
    {
        ...
        $data = $data->where('return_budget.status', '=', $approved);
        $data = $data
            ->whereNull('deleted_at')
            ->select(
                'return_budget.*',
        ...
        $data1 = DB::table('return_budget')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('return_budget.status', '=', $approved);
    ...
    public static function presidentRejectedList($company)
    {
        ...
        $data = DB::table('return_budget')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('return_budget.status', '=', $reject)
            ->whereNull('deleted_at')
            ->select(
                'return_budget.*',
                'users.name as requester_name',
        ...
        $data1 = DB::table('return_budget')
            ...
            ->where('return_budget.company_id', '=', $company)
            ->where('approve.type', '=', $type)
            //->where('return_budget.user_id', '=', Auth::id())
            ->where('return_budget.status', '=', $reject)
            ->whereNull('deleted_at');

            if (Auth::user()->role !== 1) {
                $data1 = $data1->where('return_budget.user_id', '=', Auth::id());
            }
    ...
    public static function presidentDisabledList($company)
    {
        ...
        $data = DB::table('return_budget')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('return_budget.status', '=', $disable)
            ->whereNull('deleted_at')
            ->select(
                'return_budget.*',
                'users.name as requester_name',

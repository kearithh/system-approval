...
class Survey extends Model
{
    ...
    protected $casts = [
        ...
        'operation' => 'object',
        'attachment' => 'object',
        'compare_monthly' => 'object',
        'compare_daily' => 'object',
        'compare_plan' => 'object',
    ...
    {
        $data = User
            ::leftJoin('survey_report', 'users.id', '=', 'survey_report.user_id')
            ->leftJoin('positions', 'users.position_id', '=', 'positions.id')
            ->leftJoin('approve', 'users.id', '=', 'approve.reviewer_id')
            ->where('approve.request_id', $this->id)
            ->where('approve.type', config('app.type_survey_report'))
    ...
    public function approver()
    {
        $data = User
            ...
            ->select([
                ...
                'survey_report.id as request_id',
                'survey_report.user_id as request_user_id',
                'survey_report.status as request_status',
    ...
    public function approvals()
    {
        $approvals = DB
            ::table('survey_report')
            ->join('approve', 'survey_report.id', '=', 'approve.request_id')
    ...
    public static function presidentpendingList($company)
    {
        ...
        $data = DB::table('survey_report')
            ->join('users', 'users.id', '=', 'survey_report.user_id')
            ->leftJoin('approve', 'survey_report.id', '=', 'approve.request_id')
            ->where('survey_report.company_id', '=', $company)
            ->where('approve.type', '=', $type);
        if (Auth::user()->role !== 1) {
            $data = $data->where('survey_report.user_id', '=', Auth::id());
        }

        $data = $data->where('survey_report.status', '=', $pending);
        $data = $data
            ...
            ->distinct('survey_report.id')
            ->get();

        $data1 = DB::table('survey_report')
            ->leftJoin('approve', 'survey_report.id', '=', 'approve.request_id')
            ->leftJoin('users', 'users.id', '=', 'survey_report.user_id')
            ->where('approve.type', '=', $type)
            ->where('survey_report.company_id', '=', $company)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('survey_report.status', '=', $pending)
    ...
    public static function presidentApprove($company)
    {
        ...
        $data = DB::table('survey_report')
            ...
            ->where('survey_report.status', '!=', config('app.approve_status_reject'))
            ->whereNull('deleted_at')
            ->select(
                'survey_report.*',
                'users.name as requester_name',
    ...
    public static function presidentApproved($company, $department = null)
    {
        ...
        $data = $data->where('survey_report.status', '=', $approved);
        $data = $data
            ->whereNull('deleted_at')
            ->select(
                'survey_report.*',
        ...
        $data1 = DB::table('survey_report')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('survey_report.status', '=', $approved);
    ...
    public static function presidentRejectedList($company)
    {
        ...
        $data = DB::table('survey_report')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('survey_report.status', '=', $reject)
            ->whereNull('deleted_at')
            ->select(
                'survey_report.*',
                'users.name as requester_name',
        ...
        $data1 = DB::table('survey_report')
            ...
            ->where('survey_report.company_id', '=', $company)
            ->where('approve.type', '=', $type)
            //->where('survey_report.user_id', '=', Auth::id())
            ->where('survey_report.status', '=', $reject)
            ->whereNull('deleted_at');

            if (Auth::user()->role !== 1) {
                $data1 = $data1->where('survey_report.user_id', '=', Auth::id());
            }
    ...
    public static function CountPending($company)
    {
        ...
        $data = $data
            ->where('survey_report.status', '=', $status)
            ->whereNull('deleted_at')
            ->select(
                'survey_report.id'
            )

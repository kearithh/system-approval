...
class MissionClearance extends Model
{
    use SoftDeletes;

    protected $table = 'mission_clearance';
    ...
    {
        return $this->hasMany(MissionClearanceItem::class, 'request_id', 'id');
    }

    public function items_name($id)
    {
        $id = $id ? $id : self::id;
        $data = MissionClearanceItem::where('mission_clearance_items.request_id', $id)
            ->select(
                'mission_clearance_items.desc as name'
            )
            ->get()
        ;
        return $data;
    ...
    {
        $data = User
            ::leftJoin('mission_clearance', 'users.id', '=', 'mission_clearance.user_id')
            ...
            ->select([
                ...
                'mission_clearance.id as request_id',
                'mission_clearance.user_id as request_user_id',
                'mission_clearance.status as request_status',
                'approve.user_object',
    ...
    {
        $total = DB::table('mission_clearance')
            ->where('mission_clearance.status', config('app.approve_status_approve'))
            ->whereNull('mission_clearance.deleted_at')
            ->count();
        return $total;
    }
    ...
    {
        $total = DB::table('mission_clearance')
            ->where('mission_clearance.status', config('app.approve_status_draft'))
            ->whereNull('mission_clearance.deleted_at')
            ->count();
        return $total;
    }
    ...
    {
        $total = DB::table('mission_clearance')
            ->where('mission_clearance.status', config('app.approve_status_reject'))
            ->whereNull('mission_clearance.deleted_at')
            ->count();
        return $total;
    }
    ...
    {
        $total = DB::table('mission_clearance')
            ->where('mission_clearance.status', config('app.approve_status_disable'))
            ->whereNull('mission_clearance.deleted_at')
            ->count();
        return $total;
    }
    ...
    {
        $total = DB::table('mission_clearance')
            ->whereNotNull('mission_clearance.deleted_at')
            ->count();
        return $total;
    }
    ...
    public static function presidentpendingList($company)
    {
        ...
        $data = DB::table('mission_clearance')
            ->join('users', 'users.id', '=', 'mission_clearance.user_id')
            ->leftJoin('approve', 'mission_clearance.id', '=', 'approve.request_id')
            ->where('mission_clearance.company_id', '=', $company)
            ->where('approve.type', '=', $type);
        if (Auth::user()->role !== 1) {
            $data = $data->where('mission_clearance.user_id', '=', Auth::id());
        }
        $data = $data->where('mission_clearance.status', '=', $pending);
        $data = $data
            ...
            ->distinct('mission_clearance.id')
            ->get();

        $data1 = DB::table('mission_clearance')
            ->leftJoin('approve', 'mission_clearance.id', '=', 'approve.request_id')
            ->leftJoin('users', 'users.id', '=', 'mission_clearance.user_id')
            ->where('approve.type', '=', $type)
            ->where('mission_clearance.company_id', '=', $company)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('mission_clearance.user_id', '!=', Auth::id())
            ->where('mission_clearance.status', '=', $pending)
    ...
    public static function toApproveList()
    {
        ...
        $data = DB::table('mission_clearance')
            ...
            ->whereNotIn('mission_clearance.status', [config('app.approve_status_reject'), config('app.approve_status_disable')])
            ->whereNull('deleted_at')
            // ->where('mission_clearance.updated_at', '>', '2020-05-10 23:59:00')
            ->select(
                'mission_clearance.*',
                'users.name as requester_name',
                'approve.id as approve_id',
    ...
    public static function presidentApproved($company, $department = null)
    {
        ...
        $data = $data->where('mission_clearance.status', '=', $approved);
        $data = $data
            ->whereNull('deleted_at')
            ->select(
                'mission_clearance.*',
        ...
        $data1 = DB::table('mission_clearance')
            ...
            ->where('mission_clearance.user_id', '!=', Auth::id())
            ->where('approve.type', '=', $type)
            ->where('mission_clearance.status', '=', $approved);
    ...
    public static function presidentRejectedList($company)
    {
        ...
        $data = DB::table('mission_clearance')
            ...
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('mission_clearance.user_id', '!=', Auth::id())
            ->where('mission_clearance.status', '=', $reject)
            ->whereNull('deleted_at')
            ->select(
                'mission_clearance.*',
                'users.name as requester_name',
        ...
        if (Auth::user()->role !== 1) {
            $data1 = $data1->where('mission_clearance.user_id', '=', Auth::id());
        }
    ...
    public static function presidentDisabledList($company)
    {
        ...
        $data = DB::table('mission_clearance')
            ...
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('mission_clearance.user_id', '!=', Auth::id())
            ->where('mission_clearance.status', '=', $disable)
            ->whereNull('deleted_at')
            ->select(
                'mission_clearance.*',
                'users.name as requester_name',
    ...
    {
        $approvals = DB
            ::table('mission_clearance')
            ->join('approve', 'mission_clearance.id', '=', 'approve.request_id')
    ...
    public static function CountPending($company)
    {
        ...
        $data = $data
            ->where('mission_clearance.status', '=', $status)
            ->whereNull('deleted_at')
            ->select(
                'mission_clearance.id'
            )
            ->distinct('mission_clearance.id')
            ->get();
    ...
    public static function CountToApprove($company)
    {
        ...
        $data = DB::table('mission_clearance')
            ...
            // ->where('approve.reviewer_id', '=', Auth::id())
            ->where('approve.status', '=', $pending)
            ->whereNotIn('mission_clearance.status', [$reject, $disable])
            ->where('approve.position', '!=', 'cc')
            ->whereNull('deleted_at')
            ->select(
                'mission_clearance.id'

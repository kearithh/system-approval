...
class RequestFormController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = RequestForm::getRequestByStatus($approveStatus, $approveStatus);
        $data1 = RequestForm::filter($approveStatus);
    ...
    public function approvedList()
    {
        ...
        if (in_array($type, (array)Auth::user()->view_approved_request)) {
            if (Auth::user()->role === 1) {
                ...
                $data = $data->whereNull('requests.branch_id');
            }
        } else {
    ...
    public function index1(Request $request)
    {
        ...
        $data = DB::table('requests')
            ...
            if ($request->type == 3 || $request->type == 1) {
                ...
                $data = $data->join('users', 'users.id', '=', 'requests.user_id');
            }
            $data = $data
            ->where('requests.draft', '=', 0)
...
//                    'positions.name as position_name'
        ...
        $data = $data->distinct('requests.id');
    ...
    public function create()
    {
        if (isset($_GET['request_token'])) {
            $requestForm = RequestForm::find(decrypt($_GET['request_token']));
        } else {
        ...
        return view('request.create',
            compact('reviewer', 'requester', 'requestForm', 'company', 'approver'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('request.edit',
            compact('reviewer', 'requester', 'data', 'company', 'approver', 'reviewers_short'));
    }
    ...
    public function update($id, Request $request)
    {
        ...
        if($expense->save()){
            ...
            $url = $request->root(). "/request/" . $id ."/show?menu=approved&type=Special Expense";
    ...
    public function approve(Request $request)
    {
        ...
        // Update Request
        $expense = RequestForm::find($request->request_id);
        if (Auth::id() == $expense->approver()->id) {
            $expense->status = config('app.approve_status_approve');
            $expense->save();
            // new generate code
            $codeGenerate = generateCode('requests', $expense->company_id, $id, 'SE');
        ...
        // $creater = User::leftJoin('requests', 'users.id', '=', 'requests.user_id')

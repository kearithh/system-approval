...
class RejectController extends Controller
{
    ...
    {   
        ...
        if($requestType == 'Memo'){
            $data = RequestMemo::presidentRejectedList($company);
        ...
        elseif($requestType == 'Special'){
            $data = RequestForm::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif($requestType == 'Pr_Request'){
            $data = RequestPR::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif($requestType == 'Po_Request'){
            $data = RequestPO::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif($requestType == 'GRN'){
            $data = RequestGRN::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif($requestType == 'Village'){
            $data = VillageLoan::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif($requestType == 'Withdrawal'){
            $data = WithdrawalCollateral::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'General'){
            $data = RequestHR::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'Disposal'){
            $data = Disposal::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'DamagedLog'){
            $data = DamagedLog::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'Letter'){
            $data = HRRequest::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'Loan'){
            $data = Loan::presidentRejectedList($company);
        ...
        elseif ($requestType == 'SaleAsset'){
            $data = SaleAsset::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'ReturnBudget'){
            $data = ReturnBudget::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'SendReceive'){
            $data = SendReceive::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
            return view('president.send_receive', compact('data','type', 'memo', 'special', 'general', 'disposal', 'damagedlog', 'hr_request', 'loan', 'sale_asset', 'return_budget', 'send_receive', 'mission', 'v_mission', 'training', 'request_ot'));
        }

        elseif ($requestType == 'RescheduleLoan'){
            $data = RescheduleLoan::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'Mission'){
            $data = Mission::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'MissionClearance'){
            $data = MissionClearance::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'Training'){
            $data = Training::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'RequestOT'){
            $data = RequestOT::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'Penalty'){
            $data = Penalty::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'Interest'){
            $data = Penalty::presidentRejectedListInterest($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'Wave_Association'){
            $data = Penalty::presidentRejectedListAssociation($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'EmployeePenalty'){
            $data = EmployeePenalty::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'CashAdvance'){
            $data = CashAdvance::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'Resign'){
            $data = Resign::presidentRejectedListResign($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'RequestLastDay'){
            $data = Resign::presidentRejectedListRequestLastDay($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'GeneralRequest'){
            $data = GeneralRequest::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'TransferAsset'){
            $data = TransferAsset::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'Association'){
            $data = Association::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'CustomLetter'){
            $data = CustomLetter::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);
        ...
        elseif ($requestType == 'RequestGasoline'){
            $data = RequestGasoline::presidentRejectedList($company);
            $key = $menu.'_'.$requestType.'_next_pre';
            $request->session()->put($key, @$data->next_pre);

...
class Association extends Model
{
    ...
    {
        $data = User
            ::leftJoin('associations', 'users.id', '=', 'associations.user_id')
            ->leftJoin('positions', 'users.position_id', '=', 'positions.id')
            ->leftJoin('approve', 'users.id', '=', 'approve.reviewer_id')
            ->where('approve.request_id', $this->id)
            ->where('approve.type', config('app.type_association'))
    ...
    public function approver()
    {
        $data = User
            ...
            ->select([
                ...
                'associations.id as request_id',
                'associations.user_id as request_user_id',
                'associations.status as request_status',
                'approve.user_object',
    ...
    public function approvals()
    {
        $approvals = DB
            ::table('associations')
            ->join('approve', 'associations.id', '=', 'approve.request_id')
    ...
    public static function presidentpendingList($company)
    {
        ...
        $data = DB::table('associations')
            ->join('users', 'users.id', '=', 'associations.user_id')
            ->leftJoin('approve', 'associations.id', '=', 'approve.request_id')
            ->where('associations.company_id', '=', $company)
            ->where('approve.type', '=', $type);
        if (Auth::user()->role !== 1) {
            $data = $data->where('associations.user_id', '=', Auth::id());
        }

        $data = $data->where('associations.status', '=', $pending);
        $data = $data
            ...
            ->distinct('associations.id')
            ->get();

        $data1 = DB::table('associations')
            ->leftJoin('approve', 'associations.id', '=', 'approve.request_id')
            ->leftJoin('users', 'users.id', '=', 'associations.user_id')
            ->where('approve.type', '=', $type)
            ->where('associations.company_id', '=', $company)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('associations.status', '=', $pending)
    ...
    public static function presidentApprove($company)
    {
        ...
        $data = DB::table('associations')
            ...
            ->whereNotIn('associations.status', [config('app.approve_status_reject'), config('app.approve_status_disable')])
            ->whereNull('deleted_at')
            ->select(
                'associations.*',
                'users.name as requester_name',
    ...
    public static function presidentApproved($company, $department = null)
    {
        ...
        $data = $data->where('associations.status', '=', $approved);
        $data = $data
            ->whereNull('deleted_at')
            ->select(
                'associations.*',
        ...
        $data1 = DB::table('associations')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('associations.status', '=', $approved);
    ...
    public static function presidentRejectedList($company)
    {
        ...
        $data = DB::table('associations')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('associations.status', '=', $reject)
            ->whereNull('deleted_at')
            ->select(
                'associations.*',
                'users.name as requester_name',
        ...
        $data1 = DB::table('associations')
            ...
            ->where('associations.company_id', '=', $company)
            ->where('approve.type', '=', $type)
            //->where('associations.user_id', '=', Auth::id())
            ->where('associations.status', '=', $reject)
            ->whereNull('deleted_at');

            if (Auth::user()->role !== 1) {
                $data1 = $data1->where('associations.user_id', '=', Auth::id());
            }
    ...
    public static function presidentDisabledList($company)
    {
        ...
        $data = DB::table('associations')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('associations.status', '=', $disable)
            ->whereNull('deleted_at')
            ->select(
                'associations.*',
                'users.name as requester_name',
    ...
    public static function CountPending($company)
    {
        ...
        $data = $data
            ->where('associations.status', '=', $status)
            ->whereNull('deleted_at')
            ->select(
                'associations.id'
            )
    ...
    public static function CountToApprove($company)
    {
        ...
        $data = DB::table('associations')
            ...
            // ->where('approve.reviewer_id', '=', Auth::id())
            ->where('approve.status', '=', $pending)
            ->whereNotIn('associations.status', [$reject, $disable])
            ->whereNull('deleted_at')
            ->select(
                'associations.id'
            )

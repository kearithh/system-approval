...
//        'position_id',
    ...
    public function reviewerss()
    {
        $data = User
            ...
            ->where('approve.type', config('app.type_special_expense'));
    ...
    public function reviewers()
    {
        $data = User
            ...
            ->leftJoin('positions', 'users.position_id', '=', 'positions.id')
            ->leftJoin('requests', 'approve.request_id', '=', 'requests.id')
            ->where('approve.request_id', $this->id)
            ->where('approve.type', '=', config('app.type_special_expense'))
    ...
    {
        $data = User::leftJoin('requests', 'users.id', '=', 'requests.user_id')
            ->leftJoin('positions', 'users.position_id', '=', 'positions.id')
...
//    }
    ...
    public static function reviewerName($id)
    {
        $reviewerIds = DB
            ...
            ->leftJoin('positions', 'users.position_id', '=', 'positions.id')
            ->where('type', '=', config('app.type_special_expense'))
            ->where('request_id', $id)
...
//        dd($data->toArray(), $totalPendingPrice,$totalPending, $totalApprovePrice, $totalApprove);
    ...
    public static function filter($status = null)
    {
        ...
        $data = DB::table('requests')
            ...
            ->leftJoin('approve', 'requests.id', '=', 'approve.request_id')
            ->where('approve.type', '=', $type)
            ->where('requests.user_id', '=', Auth::id());

        if ($status == $pending)
        {
            $data = $data->where('requests.status', $pending);
        }
        if ($status == $approve)
        {
            $data = $data->where('requests.status', '=', $approve);
        }
        if ($status == $reject)
        {
            $data = $data->where('requests.status', '=', $reject);
        }

        if ($status == $disable)
        {
            $data = $data->where('requests.status', '=', $disable);
        }
    ...
    public static function totalApproval($status = null, $approveStatus = null)
    {
        ...
        $totalApproval = DB
            ::table('requests')
            ->join('approve', 'requests.id', '=', 'approve.request_id')
            ->where('approve.type', '=', $type);
            if ($status) {
                $totalApproval = $totalApproval->where('requests.status', '=', $status);
            }
            if ($approveStatus) {
            ...
            ->get(['requests.id', 'approve.status'])
        ;
        if (Auth::user()->position->level == config('app.position_level_president')) {
    ...
    public static function totalApproveds()
    {
        $total = DB::table('requests')
                ...
                ->where('requests.status', config('app.approve_status_approve'))
                ->whereNull('requests.deleted_at')
                ->count();
        return $total;
    }
    ...
    public static function totalPendings()
    {
        $total = DB::table('requests')
                ->join('companies', 'companies.id', '=', 'requests.company_id')
                    ->whereNull('companies.deleted_at')
                ->where('requests.status', config('app.approve_status_draft'))
                ->whereNull('requests.deleted_at')
                ->count();
        return $total;
    }
    ...
    public static function totalCommenteds()
    {
        $total = DB::table('requests')
                ->join('companies', 'companies.id', '=', 'requests.company_id')
                    ->whereNull('companies.deleted_at')
                ->where('requests.status', config('app.approve_status_reject'))
                ->whereNull('requests.deleted_at')
                ->count();
        return $total;
    }
    ...
    public static function totalRejecteds()
    {
        $total = DB::table('requests')
                ->join('companies', 'companies.id', '=', 'requests.company_id')
                    ->whereNull('companies.deleted_at')
                ->where('requests.status', config('app.approve_status_disable'))
                ->whereNull('requests.deleted_at')
                ->count();
        return $total;
    }
    ...
    public static function totalDeleteds()
    {
        $total = DB::table('requests')
                ->join('companies', 'companies.id', '=', 'requests.company_id')
                    ->whereNull('companies.deleted_at')
                ->whereNotNull('requests.deleted_at')
                ->count();
        return $total;
    }
...
//            $data = CollectionHelper::paginate($data, $total, $pageSize);
    ...
    public static function pendingList()
    {
        ...
        $data = DB::table('requests')
            ...
            ->where('approve.type', '=', $type);
        if (@Auth::user()->role !== 1) {
            $data = $data->where('requests.user_id', '=', Auth::id());
        }
        $data = $data->where('requests.status', '=', $pending);
        $data = $data
            ->whereNull('deleted_at')
    ...
    public static function presidentpendingList($company)
    {
        ...
        $data = DB::table('requests')
            ->join('users', 'users.id', '=', 'requests.user_id')
            ->leftJoin('approve', 'requests.id', '=', 'approve.request_id')
            ->where('requests.company_id', '=', $company)
            ->where('approve.type', '=', $type);
        if (@Auth::user()->role !== 1) {
            $data = $data->where('requests.user_id', '=', Auth::id());
        }
    ...
    public static function toApproveList()
    {
        ...
        $data = DB::table('requests')
            ...
            ->whereNotIn('requests.status', [config('app.approve_status_reject'), config('app.approve_status_disable')])
            ->whereNull('deleted_at')
            ->select(
                'requests.*',
                'users.name as requester_name',
    ...
    public static function presidentApproved($company, $department = null)
    {
        ...
        $data = $data->where('requests.status', '=', $approved);
        $data = $data
            ->whereNull('deleted_at')
            ->select(
                'requests.*',
    ...
    public static function presidentRejectedList($company)
    {
        ...
        $data1 = DB::table('requests')
            ...
            if (@Auth::user()->role !== 1) {
                $data1 = $data1->where('requests.user_id', '=', Auth::id());
            }

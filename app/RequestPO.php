...
//        'position_id',
    ...
    {
        $data = RequestItemPO
            ::where('request_items_po.request_id', $id)
            ->select(
                'request_items_po.name',
                'request_items_po.unit_price',
                'request_items_po.qty',
                'request_items_po.currency'
            )
            ->get()
        ;
        return $data;
    ...
    {
        $data = User
            ::leftJoin('requests_po', 'users.id', '=', 'requests_po.user_id')
        ...
        $data = $data
            ->select([
                ...
                'requests_po.id as request_id',
                'requests_po.user_id as request_user_id',
                'requests_po.status as request_status',
    ...
    {
        $reviewers = DB::table('requests_po')
    ...
    public function reviewerst()
    {
        $data = User
            ::leftJoin('approve', 'approve.reviewer_id', '=', 'users.id')
            ->leftJoin('positions', 'users.position_id', '=', 'positions.id')
            ->leftJoin('requests_po', 'approve.request_id', '=', 'requests_po.id')
            ->where('approve.request_id', $this->id)
            ->where('approve.type', '=', config('app.type_po_request'))
...
//dd($data);
    ...
    {
        $data = User::leftJoin('requests_po', 'users.id', '=', 'requests_po.user_id')
            ->leftJoin('positions', 'users.position_id', '=', 'positions.id')
...
//    }
    ...
    {
        return $this->hasMany(RequestItemPO::class, 'request_id');
    }
...
//            }
    ...
    {
        $data = RequestPO::join('request_items_po', 'request_items_po.request_id', '=', 'requests_po.id')
            ->select(
                DB::raw('request_items_po.qty * request_items_po.unit_price as subtotal')
            )
            ->where('requests_po.id', $this->id)
            ->get();
        $data->sum('subtotal');
 ...
 */
    public static function totalPrice($id = null)
    {
        $data = RequestPO::join('request_items_po', 'request_items_po.request_id', '=', 'requests_po.id')
            ->select(
                DB::raw('request_items_po.qty * request_items_po.unit_price + ((request_items_po.qty * request_items_po.unit_price * request_items_po.vat)/100) as subtotal')
            )
    ...
    {
        $data = RequestPO
            ::join('request_items_po', 'request_items_po.request_id', '=', 'requests_po.id')
            ->join('users', 'requests_po.user_id', '=', 'users.id')
            ->select(
                DB::raw('sum((request_items_po.qty * request_items_po.unit_price) + ((request_items_po.qty * request_items_po.unit_price * request_items_po.vat)/100)) as total_price'),
                'requests_po.id',
                'requests_po.status'
            )
...
//            ->whereBetween('status', [0, 99])
            ->groupBy('requests_po.id', 'requests_po.status')
            ->get();
...
//        dd($data->toArray(), $totalPendingPrice,$totalPending, $totalApprovePrice, $totalApprove);
    ...
    public static function filter($status = null)
    {
        ...
        if ($status == $pending)
        {
            $data = $data->where('requests_po.status', $pending);
        }
        if ($status == $approve)
        {
            $data = $data->where('requests_po.status', '=', $approve);
        }
        if ($status == $reject)
        {
            $data = $data->where('requests_po.status', '=', $reject);
        }

        if ($status == $disable)
        {
            $data = $data->where('requests_po.status', '=', $disable);
        }
    ...
    public static function totalApproval($status = null, $approveStatus = null)
    {
        ...
        $totalApproval = DB
            ...
            ->where('approve.type', '=', $type);
            if ($status) {
                $totalApproval = $totalApproval->where('requests_po.status', '=', $status);
            }
            if ($approveStatus) {
            ...
            ->get(['requests_po.id', 'approve.status'])
        ;
        if (Auth::user()->position->level == config('app.position_level_president')) {
    ...
    {
        $total = DB::table('requests_po')
                ->join('companies', 'companies.id', '=', 'requests_po.company_id')
                    ->whereNull('companies.deleted_at')
                ->where('requests_po.status', config('app.approve_status_approve'))
                ->whereNull('requests_po.deleted_at')
                ->count();
        return $total;
    }
    ...
    public static function totalCommenteds()
    {
        $total = DB::table('requests_po')
                ->join('companies', 'companies.id', '=', 'requests_po.company_id')
                    ->whereNull('companies.deleted_at')
                ->where('requests_po.status', config('app.approve_status_reject'))
                ->whereNull('requests_po.deleted_at')
                ->count();
        return $total;
    }
    ...
    public static function totalRejecteds()
    {
        $total = DB::table('requests_po')
                ->join('companies', 'companies.id', '=', 'requests_po.company_id')
                    ->whereNull('companies.deleted_at')
                ->where('requests_po.status', config('app.approve_status_disable'))
                ->whereNull('requests_po.deleted_at')
                ->count();
        return $total;
    }
    ...
    public static function totalDeleteds()
    {
        $total = DB::table('requests_po')
                ->join('companies', 'companies.id', '=', 'requests_po.company_id')
                    ->whereNull('companies.deleted_at')
                ->whereNotNull('requests_po.deleted_at')
                ->count();
        return $total;
    }
...
//            $data = CollectionHelper::paginate($data, $total, $pageSize);
    ...
    public static function pendingList()
    {
        ...
        $data1 = DB::table('requests_po')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('requests_po.status', '=', $pending)
    ...
    public static function presidentpendingList($company)
    {
        ...
        $data = DB::table('requests_po')
            ->join('users', 'users.id', '=', 'requests_po.user_id')
            ->leftJoin('approve', 'requests_po.id', '=', 'approve.request_id')
            ->where('requests_po.company_id', '=', $company)
            ->where('approve.type', '=', $type);
        if (@Auth::user()->role !== 1) {
            $data = $data->where('requests_po.user_id', '=', Auth::id());
        }
    ...
    public static function toApproveList()
    {
        ...
        $data = DB::table('requests_po')
            ...
            ->whereNotIn('requests_po.status', [config('app.approve_status_reject'), config('app.approve_status_disable')])
            ->whereNull('deleted_at')
            ->select(
                'requests_po.*',
                'users.name as requester_name',
    ...
    public static function rejectedList()
    {
        ...
        $data = DB::table('requests_po')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('requests_po.status', '=', $reject)
            ->whereNull('deleted_at')
            ->select(
                'requests_po.*',
                'users.name as requester_name',
    ...
    public static function presidentRejectedList($company)
    {
        ...
        $data1 = DB::table('requests_po')
            ...
            if (@Auth::user()->role !== 1) {
                $data1 = $data1->where('requests_po.user_id', '=', Auth::id());
            }
    ...
    public static function presidentDisabledList($company)
    {
        ...
        $data = DB::table('requests_po')
            ...
            ->where('approve.type', '=', $type)
            ->where('approve.reviewer_id', '=', Auth::id())
            ->where('requests_po.status', '=', $disable)
            ->whereNull('deleted_at')
            ->select(
                'requests_po.*',
                'users.name as requester_name',

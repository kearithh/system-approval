          ...
          <div class="card card-outline card-primary">
            <div class="card-header">
                @if (\Illuminate\Support\Facades\Auth::user()->position->level != config('app.position_level_president'))
                    <a class="btn btn-xs bg-orange" href="/request_po?status=1&type=2" style="font-size: 0.85rem;">
                        Your Pending Request
                        <span class="badge badge-light">{{ $totalPendingRequest }}</span>
                    </a>
                @endif
              <a class="btn btn-xs bg-info" href="/request_po?status=1&type=3" style="font-size: 0.85rem;">
...
{{--              @include('request.search')--}}
              @include('global.search',['clear_url' => 'request_po'])
            </div>
            <!-- /.card-body -->
          </div>
          <!-- /.card -->
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-header card-header-primary">
              <h4 class="card-title ">{{ __('PO Request List') }}</h4>
              <div class="text-right">
                <a href="{{ route('request_po.create') }}" class="btn btn-sm btn-success">{{ __('Create') }}</a>
              </div>
            </div>
            <div class="table-responsive" style="padding: 0 10px">
              <table class="table table-striped">
                ...
                <th style="min-width: 130px">{{ __('តម្លៃសរុប') }}</th>
                </thead>
                <tbody>
                <?php $i = 1; ?>
                @foreach($data as $key => $item)
                  <tr>
                    <td> {{ $i++  }}</td>
                    <td class="td-actions">
                      @include('global.list_action', ['uri' => 'request_po', 'object' => $item, 'type' => config('app.type_po_request')])
                    </td>
                    <td>
                      {{ request_status($item) }}
                    </td>
                    ...
                    <td>
                      {{ reviewer_position(\App\RequestPO::reviewerName($item->id)) }}
                    </td>
                    <td>

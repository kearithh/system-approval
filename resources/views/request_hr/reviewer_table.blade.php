<div id="reviewer" style="padding: 15px; margin-bottom: 5px">
    <table class="table table-bordered">
        ...
        @foreach($reviewers as $key => $value)
            ...
            <tr>
                ...
                <td>
                    @if (is_string(@$value->position) && strpos(@$value->position, 'agree_by') !== false || is_string(@$value->position) && strpos(@$value->position, 'reviewer_short_2') !== false)
                    ...
                    @else
                        ...
                        {{ @$value->name ? @$value->name : @$value->user_name }}-{{ @$value->name_en }}
                    @endif
                </td>
                ...
                <td class="text-center">
                    @if(@$value->approved_at)
                        @if(@$data->company_id == 2 || @$data->company_id == 6) 
                            ...
                            {{ (\Carbon\Carbon::createFromTimestamp(strtotime(@$value->approved_at))->format('d-m-Y h:i:s a')) }}-{{ @$value->name_en }}
                        @else
                            {{ (\Carbon\Carbon::createFromTimestamp(strtotime(@$value->approved_at))->format('d-m-Y')) }}-{{ @$value->name_en }}
                        @endif
                    @endif
                </td>
            </tr>
    ...
    @if(@$data->clear_doc)
        <span>ឯកសារភ្ជាប់(Clear Cash Advanve)៖</span>
        @if(is_array($data->clear_doc))
            <?php $atts =  $data->clear_doc; ?>
            @foreach($atts as $doc )
                <a class="reference_link" href="{{ asset($doc->src) }}" target="_self">{{ $doc->org_name }}</a><br>
            @endforeach
        @else
            <a class="reference_link" href="{{ asset('/'.@$data->clear_doc) }}" target="_self">{{@$data->att_name}}</a>
        @endif
    @endif

...
<body style="background: #dadada">
  ...
  <div id="action_container" style="width: 1024px; margin: auto;background: white;">
    <div id="action">
      ...
      <form style="margin: 0; display: inline-block " action="">
        <div class="btn-group" role="group" aria-label="">
          @if(!can_approve_reject($data, config('app.type_village_loan')))
        ...
        <tr>
          <td class="contain">
	                ...
	                @if (@$data->forcompany->short_name_en == 'MMI')
                      ...
                      @else
                          @if(@$data->subApprover()->gender == 'M')
                            លោក{{ @json_decode(@$data->approver()->user_object)->position_name ?: @$data->subApprover()->position->name_km }}
                          @elseif(@$data->subApprover()->gender == 'F')
                            លោកស្រី{{ @json_decode(@$data->approver()->user_object)->position_name ?: @$data->subApprover()->position->name_km }}
                          @else
                            {{ @json_decode(@$data->approver()->user_object)->position_name ?: @$data->subApprover()->position->name_km }}
                          @endif
                      @endif
                  @else
            ...
            <div class="body">
	              ...
	              <b>
	                <u>១.សមាសភាពអ្នកចូលរួមដឹកសាច់ប្រាក់</u>
	              </b>
	            <table id="item">
	              <thead>
	                <tr>
	                    <td>ល.រ</td>
	                    <td style="min-width: 120px">ឈ្មោះបុគ្គលិក</td>
	                    <td>តួនាទី</td>
	                    <td>សមាសភាព</td>
	                </tr>
	              </thead>
                  ...
                  @foreach($staffs as $key => $staff)
	                    <tr>
	                        ...
	                        <td style="text-align: center;">{{ $staff->name_staff }}</td>
	                        <td style="text-align: center;">{{ $staff->position_staff }}</td>
	                        <td style="text-align: center;">{{ $staff->composition }}</td>
	                    </tr>
	                @endforeach
              ...
              <p>
                <b>
                  <u>២.ឈ្មោះអតិថិជន និងទឹកប្រាក់ដែលត្រូវដឹកជញ្ជូនទៅបើកនៅភូមិ៖	</u>
                </b>
              </p>
             <table id="item">
	              <thead>
	                <tr>
	                    <td>ល.រ</td>
	                    <td style="min-width: 120px">ឈ្មោះអតិថិជន</td>
	                    <td>លេខ CID</td>
	                    <td>ចំនួនទឹកប្រាក់បើក</td>
	                    <td style="min-width: 90px">ឈ្មោះភូមិ</td>
	                    <td style="min-width: 90px">ចម្ងាយផ្លូវ</td>
	                </tr>
	              </thead>
	              <tbody>
	                ...
	                @foreach($data->items as $key => $item)
	                    ...
	                    <tr>
	                        ...
	                        <td style="text-align: center;">{{ $item->road }}</td>
              ...
              <ul>
                <li>
                    <br><strong>មូលហេតុ</strong>៖ {{ $data->reason }}
                </li>
	            </ul>
                  ...
                  <div class="desc" style="text-align: justify; padding-bottom: 15px; text-indent : 3em;">
                      ...
                      <p>
                          ...
                          @else
                              ...
                              @endif
                          @endif 
                          មេត្តាទទួលនូវសេចក្ដីគោរពដ៏ខ្ពង់ខ្ពស់ពី{{ prifixGender($data->requester()->gender) }} ដោយក្តីអនុគ្រោះផងចុះ។
                      </p>
                  </div>
                ...
                <div style="width: {{ (100/$allCol).'%' }}">
                    @if (@$data->approver()->approve_status == config('app.approve_status_approve'))
                        ...
                        <p>
                          ...
                          {{ @json_decode(@$data->approver()->user_object)->position_name ?: $data->approver()->name }}
                        </p>
                    @else
                        <p>ថ្ងៃទី.....ខែ.....ឆ្នំា.....</p>
                        <p>អនុម័តដោយ៖</p>
  ...
  @include('global.comment_modal', ['route' =>route('village_loan.reject', $data->id)])
  @include('global.disable_modal', ['route_disable' => route('village_loan.disable', $data->id)])
  ...
  $( "#approve_btn" ).on( "click", function( event ) {
      ...
      Swal.fire({
          ...
          if (result.value) {
              ...
              $.ajax({
                  type: "POST",
                  url: "{{ action('VillageLoanController@approve', $data->id) }}",
                  data: {
                      _token: "{{ csrf_token() }}",
                      request_id: "{{ $data->id }}"
                  },

...
<body style="background: #dadada">
  ...
  <div id="action_container" style="width: 1024px; margin: auto;background: white;">
    <div id="action">
      ...
      <form style="margin: 0; display: inline-block " action="">
        <div class="btn-group" role="group" aria-label="">
          @if(!can_approve_reject($data, config('app.type_request_create_user')))
      ...
      <tbody>
        <tr>
          <td class="contain">
            ...
            <div class="header">
                <h1>ឧបសម្ព័ន្ធ</h1>
                <h1>ទម្រង់ស្នើរសុំបង្កើតឈ្មោះអ្នកប្រើប្រាស់ប្រព័ន្ធ</h1>
                <br>
            </div>
            <div class="body">
                ...
                <table class="table table-bordered text-center border">
                    ...
                    @if(@$reviewer->approve_status == config('app.approve_status_approve'))
                        <tr style="height: 100px;">
                            <td style="width: 33%">
                                {{-- <p>{{ @$reviewer->approve_comment }}</p> --}}
                                <p>បានពិនិត្យ និងអនុញ្ញាតតាមសំណើ</p>
                            </td>
                            <td style="width: 34%">
  ...
  @include('global.comment_modal', ['route' =>route('request_create_user.reject', $data->id)])
</body>
@if(! config('adminlte.enabled_laravel_mix'))
  ...
  $( "#approve_btn" ).on( "click", function( event ) {
      ...
      Swal.fire({
          ...
          if (result.value) {
              $.ajax({
                  type: "POST",
                  url: "{{ action('RequestCreateUserController@approve', $data->id) }}",
                  data: {
                      _token: "{{ csrf_token() }}",
                      request_id: "{{ $data->id }}"
                  },

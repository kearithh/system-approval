...
<body style="background: #dadada">
  ...
  <div id="action_container" style="width: 1024px; margin: auto;background: white;">
    <div id="action">
      ...
      <form style="margin: 0; display: inline-block " action="">
        <div class="btn-group" role="group" aria-label="">
          @if(!can_approve_reject($data, config('app.type_wc_request')))
      ...
      <tbody>
        <tr>
          <td class="contain">
            ...
            <div class="header">
              @if (@$data->branch_id)
                  <h1>សាខា {{ @$data->forbranch->name_km }}</h1>
              @else
            ...
            <div class="body">
                ...
                <li>
                  @foreach($data->reviewers() as $reviewer)
                      <p class="mb-0"><strong>តាមរយៈ</strong>៖ {{ @json_decode(@$reviewer->user_object)->position_name ?: $reviewer->position_name }}</p>
                  @endforeach 
	              </li>
                <li>
                ...
                @if ($data->reason)
                  <li>
                    <strong>គោលបំណង</strong>៖ {{ $data->reason }}
                  </li>
                @endif
	            </ul>
	            <p>
                តបតាមកម្មវត្ថុ និងគោលបំណងដូចបានជម្រាបជូនខាងលើ {{prifixGender($data->requester()->gender)}}មានកត្តិយស
                @if(@$data->approver()->position_level == config('app.position_level_president'))
                    ជម្រាបជូនលោក{{ @$data->forcompany->approver }}
                @else
                    @if(@$data->approver()->gender == 'M')
                      លោក{{ @json_decode(@$data->approver()->user_object)->position_name ?: @$data->approver()->position->name_km }}
                    @elseif(@$data->approver()->gender == 'F')
                    ...
                    @endif
                @endif
                មេត្តាត្រួតពិនិត្យ និងអនុម័តលើសំណើដែលមានព័ត៌មានដូចខាងក្រោម៖
	            </p>
              <!-- for mmi -->
	              ...
	              <tbody>
	                @foreach($data->items as $key => $item)
	                    <tr>
                            ...
                            ថ្ងៃទី{{ khmer_number(\Carbon\Carbon::createFromTimestamp(strtotime($item->date))->format('d')) }}
                            ខែ{{ khmer_month(\Carbon\Carbon::createFromTimestamp(strtotime($item->date))->format('m')) }}
                            ឆ្នំា{{ khmer_number(\Carbon\Carbon::createFromTimestamp(strtotime($item->date))->format('Y')) }}
                          </td>
	                    </tr>
	                @endforeach
	              </tbody>
              ...
              <br>
              <p>
	                អាស្រ័យហេតុដូចបានជម្រាបជូនខាងលើ
                  @if(@$data->approver()->position_level == config('app.position_level_president'))
                      សូមលោក{{ @$data->forcompany->approver }} មេត្តាពិនិត្យ និងសម្រេចតាមការគួរ។
                  @else
                      @if(@$data->approver()->gender == 'M')
                        លោក{{ @json_decode(@$data->approver()->user_object)->position_name ?: @$data->approver()->position->name_km }}
                      @elseif(@$data->approver()->gender == 'F')
                      ...
                      @endif
                  @endif
                  មេត្តាពិនិត្យ និងសម្រេចតាមការគួរ។
	            </p>
              <p>
                  @if(@$data->approver()->position_level == config('app.position_level_president'))
                      សូមលោក{{ @$data->forcompany->approver }} មេត្តាពិនិត្យ និងសម្រេចតាមការគួរ។
                  @else
                      ...
                      @endif
                  @endif
                  ទទួលនូវការគោរពដ៏ខ្ពង់ខ្ពស់អំពីខ្ញុំ។
              </p>
  ...
  @include('global.comment_modal', ['route' =>route('withdrawal_collateral.reject', $data->id)])
  @include('global.disable_modal', ['route_disable' => route('withdrawal_collateral.disable', $data->id)])
  ...
  $( "#approve_btn" ).on( "click", function( event ) {
      ...
      Swal.fire({
          ...
          if (result.value) {
              ...
              $.ajax({
                  type: "POST",
                  url: "{{ action('WithdrawalCollateralController@approve', $data->id) }}",
                  data: {
                      _token: "{{ csrf_token() }}",
                      request_id: "{{ $data->id }}"
                  },

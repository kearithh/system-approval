<div class="card-body ">
    ...
    <?php $item = $data->items->first(); ?>
        ...
        <div class="row">
            <div class="col-sm-2">
              <label>ប្រភេទក្រដាសប្រាក់ផ្លាស់ប្តូរ<span style='color: red'>*</span></label>
            </div>
            <div class="col-sm-10 form-group">
              <select class="form-control select2" required name="type_exchange">
                <option value=""><< ជ្រើសរើស >></option>
                <option value="1" @if($item->currency_exchange == 'KHR') selected @endif > ប្តូរពីរៀលទៅដុល្លា </option>
                <option value="2" @if($item->currency_exchange == 'USD') selected @endif > ប្តូរពីដុល្លាទៅរៀល </option>
              </select>
            </div>
        </div>

        <div class="row">
            <label class="col-sm-2 col-form-label">ទឹកប្រាក់ប្តូរចេញ<span style='color: red'>*</span></label>
            <div class="col-sm-10">
              <div class="form-group{{ $errors->has('exchange ') ? ' has-danger' : '' }}">
                <input 
                        type="number" 
                        id="exchange"
                        class="form-control{{ $errors->has('exchange') ? ' is-invalid' : '' }}"
                        name="exchange"
                        value="{{ $item->money_exchange }}" 
                        required
                >
              </div>
            </div>
        ...
        <div class="row">
            <label class="col-sm-2 col-form-label">អត្រាប្តូរប្រាក់<span style='color: red'>*</span></label>
            <div class="col-sm-10">
        ...
        <div class="row">
            <label class="col-sm-2 col-form-label">ទឹកប្រាក់ប្តូរចូល<span style='color: red'>*</span></label>
            <div class="col-sm-10">
              <div class="form-group{{ $errors->has('remittance') ? ' has-danger' : '' }}">
                <input 
                        type="number" 
                        id="remittance"
                        class="form-control{{ $errors->has('remittance') ? ' is-invalid' : '' }}"
                        name="remittance"
                        value="{{ $item->money_remittance }}" 
                        required
                >
              </div>
            </div>
...
<div class="card-footer">
    @if ($data->status == config('app.approve_status_reject'))
        <button
            ...
            @endif
            type="submit"
            formaction="{{ route('general_request.update_exchange_money', $data->id) }}"
            name="resubmit"
            value="1"
            form="requestForm"
            class="btn btn-info">

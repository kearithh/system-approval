                        ...
                        <div class=" @if(@$approved) col-md-8 col-lg-9  @else col-md-12 col-lg-12 @endif"> 
                            ...
                            <div class="card">
                                <div class="card-header card-header-primary">
                                    <h4 class="card-title "><strong>Transfer Asset List</strong></h4>
                                </div>
                                <div class="table-responsive" style="padding: 0 10px">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <th style="min-width: 245px; text-align: center;">ទ្រព្យសម្បត្តិផ្ទេរ</th>
                                            ...
                                            @if($data->count())
                                                ...
                                                @foreach($data as $key => $item)
                                                    <tr>
                                                        <td> {{ $i++ }}</td>
                                                        <td class="td-actions">
                                                            @include('global.list_action', ['uri' => 'transfer_asset', 'object' => $item, 'type' => config('app.type_transfer_asset')])
                                                        </td>
                                                        <td>{{ request_status($item) }}</td>
                                                        <td>
                                                            {{ @items(\App\TransferAsset::items_name($item->id)) }}
                                                        </td>
                                                        <td>{{ $item->requester_name }}</td>
                                                        <td>
                                                            {{ @reviewer_position(\App\TransferAsset::reviewerNames($item->id)) }}
                                                        </td>
                                                        <td>
                                                            {{ @approver_position(\App\TransferAsset::approverName($item->id)) }}
                                                        </td>

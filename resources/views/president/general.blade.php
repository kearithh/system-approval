              ...
              <div class="card">
                ...
                <div class="table-responsive" style="padding: 0 10px">
                  <table class="table table-striped table-hover">
                    <thead class="">
                    <th style="min-width: 50px">ល.រ<br>No</th>
                    <th style="min-width: 100px">{{ __('សកម្ម') }}<br>Setting</th>
                    <th style="min-width: 100px;">{{ __('ស្ថានភាព') }}<br>Status</th>
                    <th style="min-width: 120px;">{{ __('កូដ') }}<br>Code</th>
                    <th style="min-width: 300px; text-align: center;">{{ __('ស្នើរសុំចំណាយ') }}<br>Request item</th>
                    <th style="min-width: 152px;">{{ __('ស្នើសំុដោយ') }}<br>Request by</th>
                    <th style="min-width: 320px;">{{ __('ពិនិត្យ និងបញ្ជូនបន្តដោយ') }}<br>Initial approved by </th>
                    <th style="min-width: 240px"> {{ __('អនុម័តដោយ') }}<br>Final Approved by</th>
                    <th style="min-width: 102px">ថ្ងៃស្នើ<br>Date request</th>
                    <th style="min-width: 102px">{{ __('តម្លៃសរុប') }}<br>Total price </th>
                    </thead>
                    <tbody>
                      @if($data->count())
                        <?php $i = 1; ?>
                        @foreach($data as $key => $item)
                          <tr>
                            ...
                            <td>{{ @showArrayCode($item->code) }}</td>
                            <td>
                                {{ @items(\App\RequestHR::items_name($item->id)) }}
                            </td>
                            ...
                            <td>
                              {{ @approver_position(\App\RequestHR::approverName($item->id)) }}
                            </td>

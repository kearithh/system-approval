                            ...
                            <div class="card">
                                ...
                                <div class="table-responsive" style="padding: 0 10px">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <th style="min-width: 145px; text-align: center;">ចំណាយសរុប</th>
                                            ...
                                            @if($data->count())
                                                ...
                                                @foreach($data as $key => $item)
                                                    <tr>
                                                        <td> {{ $i++ }}</td>
                                                        <td class="td-actions">
                                                            @include('global.list_action', ['uri' => 'request_gasoline', 'object' => $item, 'type' => config('app.type_request_gasoline')])
                                                        </td>
                                                        <td>{{ request_status($item) }}</td>
                                                        <td style="text-align: center;">{{ number_format(@$item->total_expense) }}</td>
                                                        <td>{{ $item->requester_name }}</td>
                                                        <td>{{ @reviewer_position(\App\RequestGasoline::reviewerName($item->id)) }}</td>
                                                        <td>{{ @approver_position(\App\RequestGasoline::approverName($item->id)) }}</td>
                                                        <td>{{ @reviewer_position(\App\RequestGasoline::ccName($item->id)) }}</td>

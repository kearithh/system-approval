                        ...
                        <div class=" @if(@$approved) col-md-8 col-lg-9  @else col-md-12 col-lg-12 @endif"> 
                            ...
                            <div class="card">
                                <div class="card-header card-header-primary">
                                    <h4 class="card-title "><strong>Request Disable User List</strong></h4>
                                </div>
                                <div class="table-responsive" style="padding: 0 10px">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <th style="min-width: 245px; text-align: center;">កម្មវត្ថុ</th>
                                        <th style="min-width: 152px;">ស្នើសំុដោយ</th>
                                        <th style="min-width: 320px;">ត្រួតពិនិត្យ និងទទួលដោយ</th>
                                        <th style="min-width: 245px;">អនុម័តដោយ</th>
                                        <th style="min-width: 200px;">ចម្លងជូន</th>
                                        ...
                                        <tbody>
                                            @if($data->count())
                                                ...
                                                @foreach($data as $key => $item)
                                                    <?php $obj = json_decode($item->request_object) ?>
                                                    <tr>
                                                        <td> {{ $i++ }}</td>
                                                        <td class="td-actions text-center">
                                                            @include('global.list_action', ['uri' => 'request_disable_user', 'object' => $item, 'type' => config('app.type_request_disable_user')])
                                                        </td>
                                                        <td>{{ @request_status($item) }}</td>
                                                        <td>{{ @$item->purpose }}</td>
                                                        <td>{{ @$item->requester_name }}</td>
                                                        <td>{{ @reviewer_position(\App\RequestDisableUser::reviewerName($item->id)) }}</td>
                                                        <td>{{ @approver_position(\App\RequestDisableUser::approverName($item->id)) }}</td>
                                                        <td>{{ @reviewer_position(\App\RequestDisableUser::ccNames($item->id)) }}</td>

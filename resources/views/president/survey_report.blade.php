                        ...
                        <div class=" @if(@$approved) col-md-8 col-lg-9  @else col-md-12 col-lg-12 @endif"> 
                            ...
                            <div class="card">
                                <div class="card-header card-header-primary">
                                    <h4 class="card-title "><strong>Survey Report List</strong></h4>
                                </div>
                                <div class="table-responsive" style="padding: 0 10px">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <th style="min-width: 90px;">{{ __('សាខា') }}</th>
                                            ...
                                            @if($data->count())
                                                ...
                                                @foreach($data as $key => $item)
                                                    <tr>
                                                        <td> {{ $i++ }}</td>
                                                        <td class="td-actions">
                                                            @include('global.list_action', ['uri' => 'survey_report', 'object' => $item, 'type' => config('app.type_survey_report')])
                                                        </td>
                                                        <td>{{ request_status($item) }}</td>
                                                        <td>{{ @(\App\Survey::branchName(@$item->branch_id)) }}</td>
                                                        <td>{{ $item->requester_name }}</td>
                                                        <td>{{ @reviewer_position(\App\Survey::reviewerNames($item->id)) }}</td>
                                                        <td>{{ @approver_position(\App\Survey::approverName($item->id)) }}</td>

                            ...
                            <div class="card">
                                ...
                                <div class="table-responsive" style="padding: 0 10px">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <th style="min-width: 300px;">{{ __('ចម្លងជូន') }}</th>
                                            ...
                                            @if($data->count())
                                                ...
                                                @foreach($data as $key => $item)
                                                    <tr>
                                                        <td> {{ $i++ }}</td>
                                                        <td class="td-actions">
                                                            @include('global.list_action', ['uri' => 'mission', 'object' => $item, 'type' => config('app.type_mission')])
                                                        </td>
                                                        <td>{{ request_status($item) }}</td>
                                                        <td>{{ @$item->purpose }}</td>
                                                        <td>{{ $item->requester_name }}</td>
                                                        <td>{{ @reviewer_position(\App\Mission::reviewerNames($item->id)) }}</td>
                                                        <td>{{ @approver_position(\App\Mission::approverName($item->id)) }}</td>
                                                        <td>{{ @reviewer_position(\App\Mission::ccName($item->id)) }}</td>

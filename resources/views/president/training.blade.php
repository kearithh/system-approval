                        ...
                        <div class=" @if(@$approved) col-md-8 col-lg-9  @else col-md-12 col-lg-12 @endif"> 
                            ...
                            <div class="card">
                                <div class="card-header card-header-primary">
                                    <h4 class="card-title "><strong>Training List</strong></h4>
                                </div>
                                            ...
                                            @if($data->count())
                                                ...
                                                @foreach($data as $key => $item)
                                                    <tr>
                                                        <td> {{ $i++ }}</td>
                                                        <td class="td-actions">
                                                            @include('global.list_action', ['uri' => 'training', 'object' => $item, 'type' => config('app.type_training')])
                                                        </td>
                                                        <td>{{ request_status($item) }}</td>
                                                        <td class="text-center">{{ @showArrayCode($item->code) }}</td>
                                                        <td>{{ $item->subject }}</td>
                                                        <td>{{ $item->requester_name }}</td>
                                                        <td>{{ @reviewer_position(\App\Training::reviewerNames($item->id)) }}</td>
                                                        <td>{{ @approver_position(\App\Training::approverName($item->id)) }}</td>

          ...
          <div class="card card-outline card-primary">
            <div class="card-header">
                @if (\Illuminate\Support\Facades\Auth::user()->position->level != config('app.position_level_president'))
                    <a class="btn btn-xs bg-orange" href="/withdrawal_collateral?status=1&type=2" style="font-size: 0.85rem;">
                        Your Pending Request
                        <span class="badge badge-light">{{ $totalPendingRequest }}</span>
                    </a>
                @endif
              <a class="btn btn-xs bg-info" href="/withdrawal_collateral?status=1&type=3" style="font-size: 0.85rem;">
...
{{--              @include('request.search')--}}
              @include('global.search',['clear_url' => 'withdrawal_collateral'])
            </div>
            <!-- /.card-body -->
          </div>
          <!-- /.card -->
            ...
            <div class=" @if(@$approved) col-md-8 col-lg-9  @else col-md-12 col-lg-12 @endif">
              ...
              <div class="card">
                <div class="card-header card-header-primary">
                  <h4 class="card-title "><strong>Collateral Withdrawal List</strong></h4>
                </div>
                <div class="table-responsive" style="padding: 0 10px">
                  <table class="table table-striped table-hover">
                    ...
                    <th style="min-width: 100px">កាលបរិច្ឆេទស្នើសុំដក</th>
                      ...
                      @if($data->count())
                        ...
                        @foreach($data as $key => $item)
                          <tr>
                            <td> {{ $i++  }}</td>
                            <td class="td-actions">
                              @include('global.list_action', ['uri' => 'withdrawal_collateral', 'object' => $item, 'type' => config('app.type_special_expense')])
                            </td>
                            <td>
                              {{ request_status($item) }}
                            </td>
                            ...
                            <td>
                              {{ reviewer_position(\App\WithdrawalCollateral::reviewerName($item->id)) }}
                            </td>
                            <td>
                              {{ @approver_position(\App\WithdrawalCollateral::approverName($item->id)) }}
                            </td>
                            <td>

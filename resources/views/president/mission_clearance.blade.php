                                            ...
                                            @if($data->count())
                                                ...
                                                @foreach($data as $key => $item)
                                                    <tr>
                                                        <td> {{ $i++ }}</td>
                                                        <td class="td-actions">
                                                            @include('global.list_action', ['uri' => 'mission_clearance', 'object' => $item, 'type' => config('app.type_mission_clearance')])
                                                        </td>
                                                        <td>{{ request_status($item) }}</td>
                                                        <td style="text-align: center;">{{ number_format(@$item->total) }}</td>
                                                        <td>{{ $item->requester_name }}</td>
                                                        <td>{{ @reviewer_position(\App\MissionClearance::reviewerName($item->id)) }}</td>
                                                        <td>{{ @approver_position(\App\MissionClearance::approverName($item->id)) }}</td>
                                                        <td>{{ @reviewer_position(\App\MissionClearance::ccName($item->id)) }}</td>

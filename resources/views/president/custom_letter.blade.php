                        ...
                        <div class=" @if(@$approved) col-md-8 col-lg-9  @else col-md-12 col-lg-12 @endif"> 
                            ...
                            <div class="card">
                                <div class="card-header card-header-primary">
                                    <h4 class="card-title "><strong>Custom Letter List</strong></h4>
                                </div>
                                <div class="table-responsive" style="padding: 0 10px">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <th style="min-width: 200px;">កម្មវត្ថុ</th>
                                            ...
                                            @if($data->count())
                                                ...
                                                @foreach($data as $key => $item)
                                                    <tr>
                                                        <td> {{ $i++ }}</td>
                                                        <td class="td-actions">
                                                            @include('global.list_action', ['uri' => 'custom_letter', 'object' => $item, 'type' => config('app.type_custom_letter')])
                                                        </td>
                                                        <td>{{ request_status($item) }}</td>
                                                        <td>{{ @$item->requester_name }}</td>
                                                        <td>{{ @$item->purpose }}</td>
                                                        <td>{{ @reviewer_position(\App\CustomLetter::reviewerNames($item->id)) }}</td>
                                                        <td>{{ @approver_position(\App\CustomLetter::approverName($item->id)) }}</td>
                                                        <td>{{ @reviewer_position(\App\CustomLetter::ccNames($item->id)) }}</td>

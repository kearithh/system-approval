...
<body style="background: #dadada">
  ...
  <div id="action_container" style="width: 1024px; margin: auto;background: white;">
    <div id="action">
      ...
      <form style="margin: 0; display: inline-block " action="">
        <div class="btn-group" role="group" aria-label="">
          @if(!can_approve_reject($data, config('app.return_budget')))
    ...
    @include('global.rerviewer_table', ['reviewers' =>
        $data->reviewers()->push($data->receiver())->push($data->approver())
    ])
  </div>
      ...
      <tbody>
        <tr>
          <td class="contain">
            ...
            <div class="sign">
              @include('return_budget.partials.approve_section')
            </div>
          </td>
        </tr>
      </tbody>
  ...
  @include('global.comment_modal', ['route' =>route('return_budget.reject', $data->id)])
  @include('global.disable_modal', ['route_disable' => route('return_budget.disable', $data->id)])
  ...
  $( "#approve_btn" ).on( "click", function( event ) {
      ...
      Swal.fire({
          ...
          if (result.value) {
              ...
              $.ajax({
                  type: "POST",
                  url: "{{ action('ReturnBudgetController@approve', $data->id) }}",
                  data: {
                      _token: "{{ csrf_token() }}",
                      request_id: "{{ $data->id }}"
                  },

<div class="table-responsive">
    <table class="table table-hover">
        ...
        <tbody>
        <?php $total = 0; ?>
        @foreach($requestGRN->items as $item)
            <tr>
                <td class="td-actions">
                    <form id="item_form" action="{{ route('request_item.destroy', $item->id) }}" method="POST">
                        @csrf
                        <button
                                form="item_form"
                                type="button"
            ...
            <?php $total += $item->qty * $item->unit_price + (($item->qty * $item->unit_price * $item->vat)/100)?>
        @endforeach

      ...
      <div class="row">
        ...
        <div class="col-md-12">
          <form
              id="requestForm"
              method="POST"
              action="{{ route('request_disable_user.update', $data->id) }}"
              enctype="multipart/form-data"
              class="form-horizontal">
            @csrf
            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">កែប្រែទម្រង់ស្នើរសុំបិទ/លុប និងកែប្រែអ្នកប្រើប្រាស់ប្រព័ន្ធ</h4>
              ...
              <div class="card-body ">
                    ...
                    <div class="col-md-2">
                      <label>
                        ...
                        <i class="fa fa-xs fa-question-circle tooltipsign" data-toggle="tooltip"
                           title="តំណភ្ជាប់ជាមួយនឹងសំណើ Last day របស់បុគ្គលិក"
                           data-placement="top"></i>
                      </label>
                    </div>
                    <div class="col-md-10 form-group">
                      <select class="form-control select2" name="resign_id" id="resign_id">
                        <option value=""><< ជ្រើសរើស >></option>
                        @foreach($resign as $key => $value)
                          <option value="{{ $value->id }}" @if ($data->resign_id == $value->id) selected @endif >{{ $value->name }}</option>
                        @endforeach
                      </select>
                    </div>
                  </div>
                  ...
                  <div class="row">
                    <div class="col-md-2">
                      <label>ប្រភេទនៃមូលហេតុ<span style='color: red'>*</span></label>
                    </div>
                    <div class="col-md-10 form-group">
                      <select class="form-control select2" name="type_reason[]" multiple>
                        @foreach(config('app.types_disable_user') as $key => $value)
                          <option value="{{ $key }}" @if(@in_array($key, @$data->type_reason)) selected @endif>
                            {{ $value }}
                          </option>
                        @endforeach
                      </select>
                  ...
                  <div class="request_resign_show">
                    <div class="row">
                      <div class="col-sm-2">
                        <label>មូលហេតុនៃការស្នើសុំ</label>
                      </div>
                      <div class="col-sm-10 form-group">
                        <textarea class="form-control" rows="3" name="reason">{{$data->reason}}</textarea>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="row">
                    <div class="col-md-2">
                      <label>ចម្លងជូន(CC)
                        <i class="fa fa-xs fa-question-circle tooltipsign" data-toggle="tooltip"
                           title="ផ្នែកពាក់ព័ន្ធដែលជួយសម្របសម្រួលក្នុងការលុប user ចេញពីប្រព័ន្ធផ្សេងៗ..."
                           data-placement="top"></i>
                      </label>
                    </div>  
                    <div class="col-md-10 form-group">
...
@include('request_disable_user.partials.add_more_js')

...
<body style="background: #dadada">
  ...
  <div id="action_container" style="width: 1024px; margin: auto;background: white;">
    <div id="action">
      ...
      <form style="margin: 0; display: inline-block " action="">
        <div class="btn-group" role="group" aria-label="">
            @if(!can_approve_reject($data, config('app.training')))
        ...
        <tr>
          <td class="contain">
                    ...
                    <tr>
                        <td colspan="2">
                            <table class="table table-bordered text-center">
                                <tr class="table-primary" style="height: 40px">
                                    <th style="min-width: 50px;">ល.រ</th>
                                    <th style="min-width: 100px;">តួនាទី</th>
                                    <th style="min-width: 100px;">វគ្គបណ្តុះបណ្តាល</th>
                                    <th style="min-width: 100px;">កាលបរិច្ឆេទ</th>
                                    <th style="min-width: 100px;">ម៉ោង</th>
                                    <th style="min-width: 80px;">ចំនួនថ្ងៃ</th>
                                    <th style="min-width: 180px;">ទីតាំងបណ្តុះបណ្តាល</th>
                                </tr>
                                <?php $i = 1; ?>
                                @foreach($trainingItem as $key => $item)
                                    <tr style="height: 40px">
                                        <td>{{ $i++ }}</td>
                                        <td>{{ $item->position }}</td>
                                        <td>{{ $item->course }}</td>
                                        <td>
                                            {{(\Carbon\Carbon::createFromTimestamp(strtotime($item->from_date))->format('d/m/Y'))}}
                                            @if($item->to_date)
                                              - {{(\Carbon\Carbon::createFromTimestamp(strtotime($item->to_date))->format('d/m/Y'))}}
                                            @endif
                                        </td>
                                        <td>
                                            @if (@$item->from_time != null || @$item->to_time != null)
                                                @if (@$item->from_time != null)
                                                    {{(\Carbon\Carbon::createFromTimestamp(strtotime($item->from_time))->format('h:i A'))}}
                                                @endif
                                                ...
                                                @if (@$item->to_time != null)
                                                    {{(\Carbon\Carbon::createFromTimestamp(strtotime($item->to_time))->format('h:i A'))}}
                                                @endif
                                            @else
                                                N/A
                                            @endif
            ...
            <div class="signature text-center">
                @include('training.partials.approve_section')
            </div>
          </td>
        </tr>
      </tbody>
  ...
  @include('global.comment_modal', ['route' =>route('training.reject', $data->id)])
  @include('global.disable_modal', ['route_disable' => route('training.disable', $data->id)])
  ...
  $( "#approve_btn" ).on( "click", function( event ) {
      ...
      Swal.fire({
          ...
          if (result.value) {
              ...
              $.ajax({
                  type: "POST",
                  url: "{{ action('TrainingController@approve', $data->id) }}",
                  data: {
                      _token: "{{ csrf_token() }}",
                      request_id: "{{ $data->id }}"
                  },
